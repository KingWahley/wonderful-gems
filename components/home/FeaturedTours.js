      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {tours.map((tour) => (
            ...
            >
              <Link href={`/tours/${tour.slug || tour.id}`} className="relative aspect-[4/3] w-full overflow-hidden block bg-cream-200">
                {tour.heroImage ? (
                  <Image 
                    src={tour.heroImage}
                    alt={tour.title}
              ...
              <div className="p-8 flex flex-col flex-grow">
                ...
                <h3 className="font-serif text-2xl text-charcoal-900 mb-4 leading-tight font-bold group-hover:text-coral-500 transition-colors">
                  <Link href={`/tours/${tour.slug || tour.id}`}>{tour.title}</Link>
                </h3>
                ...
                <div className="mt-4 pt-2">
                  ...
                  >
                    View Experience <span className="text-lg leading-none">&rarr;</span>
                  </Link>
                </div>
              </div>
            </div>
