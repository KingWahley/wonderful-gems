...
export default async function PlanWithMePage() {
  ...
  if (!settings && packages.length === 0) {
    return (
      <div className="pt-32 pb-24 bg-cream-100 min-h-screen flex items-center justify-center">
        <p className="text-charcoal-500 italic">Planning services are not configured yet in settings.</p>
      </div>
    );
  }
  ...
  const faqBadge = settings?.faqBadge;
  const faqTitle = settings?.faqTitle;
