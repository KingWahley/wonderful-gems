...
export default async function MiniGuidesPage() {
  ...
  const pocketGuides = guides.filter(g => g.type === "pocket");
  const itineraryGuides = guides.filter(g => g.type === "itinerary");
    ...
    <div className="pt-32 pb-24 bg-cream-100 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-24">
          ...
          {pocketGuides.length === 0 ? (
            <div className="p-12 text-center text-charcoal-800/60 text-sm font-medium bg-white rounded-[24px] border border-charcoal-900/10">
              No pocket guides published yet.
            </div>
          ) : (
        ...
        <div>
          ...
          {itineraryGuides.length === 0 ? (
            <div className="p-12 text-center text-charcoal-800/60 text-sm font-medium bg-white rounded-[24px] border border-charcoal-900/10">
              No itinerary guides published yet.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {itineraryGuides.map((guide) => (
                <div key={guide.id} className="bg-white rounded-[20px] overflow-hidden group flex flex-col border border-charcoal-900/5 shadow-sm">
                  ...
                  <div className="p-8 flex flex-col flex-grow">
                    <span className="text-charcoal-900/50 font-bold text-[10px] tracking-[0.2em] uppercase mb-4 block leading-relaxed line-clamp-3">
                      {guide.excerpt}
                    </span>
