...
export async function fetchSettings(key) {
  try {
    ...
    if (response.error) {
      ...
      console.warn(`Supabase site_settings read error for key "${key}", falling back to code default:`, response.error.message);
    ...
    console.warn(`Exception reading site_settings for key "${key}", falling back to code default:`, err);
  }
...
export async function saveSettings(key, value) {
  try {
    ...
    console.error(`Failed to save settings for key "${key}":`, err);
    throw err;
  }
}
