...
const handleResponse = ({ data, error }) => {
  if (error) {
    // Handle error objects with non-enumerable fields by extracting them explicitly
    const errorDetails = typeof error === 'object' && error !== null 
      ...
      : {};
    const msg = error.message || errorDetails.message || JSON.stringify(errorDetails) || JSON.stringify(error) || "Unknown Supabase error";
    
    // Log as a warning instead of console.error to avoid triggering Next.js critical dev overlays,
    // letting page-level components catch the error and show custom UI states gracefully.
    console.warn("Supabase query warning:", msg, error);
...
export async function fetchTours() {
  ...
  return data.map(tour => {
    ...
    const rawBookingLink = extra.bookingLink || tour.bookingLink || "";
    return {
      ...tour,
      ...extra,
      bookingLink: formatExternalLink(rawBookingLink),
...
export async function saveTour(tour) {
  ...
  const extraFields = {
    ...
    city: cleaned.city || "",
    bookingLink: formatExternalLink(cleaned.bookingLink || ""),
  ...
  const rawBookingLink = extra.bookingLink || returnedTour.bookingLink || "";
  return {
    ...returnedTour,
    ...extra,
    bookingLink: formatExternalLink(rawBookingLink),
...
export async function deleteInquiry(id) {
  ...
  if (!response.data || response.data.length === 0) {
    throw new Error("No rows affected. The database record could not be found or you do not have permission to delete it. Please check your Supabase Row Level Security (RLS) policies.");
  }
  return true;
}
